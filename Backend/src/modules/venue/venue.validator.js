...
const CreateVenueDTO = Joi.object({
  ...
  venueType: Joi.string().valid('wedding', 'conference', 'party', 'exhibition', 'other').required(),
  contactPhone: Joi.string().max(32).required(),
  contactEmail: Joi.string().email().max(128).required(),
  location: Joi.object({
...
const UpdateVenueDTO = Joi.object({
  ...
  gallery: Joi.array().items(Joi.string().uri()).optional(),
  services: Joi.array().items(Joi.string()).optional(),
  venueType: Joi.string().valid('wedding', 'conference', 'party', 'exhibition', 'other').optional(),
  contactPhone: Joi.string().max(32).optional(),
  contactEmail: Joi.string().email().max(128).optional(),
  location: Joi.object({
